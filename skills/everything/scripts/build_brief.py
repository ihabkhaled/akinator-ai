...
Usage:
    python skills/everything/scripts/build_brief.py            # dry run, report what would change
    python skills/everything/scripts/build_brief.py --write    # write the brief and the index
    python skills/everything/scripts/build_brief.py --check    # exit 1 if drifted
    python skills/everything/scripts/build_brief.py --explain  # show the ranking and what fit
...
def main(argv: list[str] | None = None) -> int:
    ...
    for rel in sorted(drifted):
        print(f"drifted {rel}")
    if drifted:
        print("\nFix with: python skills/everything/scripts/build_brief.py --write")
        return 1
